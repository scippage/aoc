...
std::vector<beam> neighbors(beam& vec) {
    ...
    switch (dir) {
        case Direction::left: {
            if (steps < 10) beams.push_back(std::make_tuple(std::make_pair(i, j-1), Direction::left, steps+1));
            if (steps >= 4) {
        ...
        case Direction::right: {
            if (steps < 10) beams.push_back(std::make_tuple(std::make_pair(i, j+1), Direction::right, steps+1));
            if (steps >= 4) {
                beams.push_back(std::make_tuple(std::make_pair(i-1, j), Direction::up, 1));
                beams.push_back(std::make_tuple(std::make_pair(i+1, j), Direction::down, 1));
            }
        ...
        case Direction::up: {
            if (steps < 10) beams.push_back(std::make_tuple(std::make_pair(i-1, j), Direction::up, steps+1));
            if (steps >= 4) {
        ...
        case Direction::down: {
            if (steps < 10) beams.push_back(std::make_tuple(std::make_pair(i+1, j), Direction::down, steps+1));
            if (steps >= 4) {
                beams.push_back(std::make_tuple(std::make_pair(i, j-1), Direction::left, 1));
                beams.push_back(std::make_tuple(std::make_pair(i, j+1), Direction::right, 1));
            }
...
int dijkstras(grid& g, std::set<beam>& visited, std::map<beam, int>& cost, beam_queue& queue) {
    while (!queue.empty()) {
        ...
        if (i == g.size()-1 && j == g[0].size()-1) {
            if (steps >= 4) return cost[vec] + g[i][j] - g[0][0];
            else continue;
        }
...
int main() {
    ...
    beam_queue queue;
    priority_beam start1 = std::make_pair(
        0,
        std::make_tuple(std::make_pair(0, 0), Direction::right, 0)
    );
    priority_beam start2 = std::make_pair(
        0,
        std::make_tuple(std::make_pair(0, 0), Direction::down, 0)
    );
    queue.push(start1);
    queue.push(start2);
