...
paths = set()
def search(curr, end, path, visited, repeat):
    if curr in visited:
        if repeat or curr.pt == 'start' or curr.pt == 'end':
            return
        else:
            repeat = True
    if curr == end:
    ...
    for neighbor in curr.neighbors:
        search(neighbor, end, path + [neighbor], copy(visited), repeat)

search(nodes['start'], nodes['end'], [nodes['start']], set(), False)
