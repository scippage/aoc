...
class Brick {
    public:
        ...
        int x2, y2, z2;
        std::string name;
        std::set<BrickPtr> supports;  // edges in
        std::set<BrickPtr> dependers;  // edges out
...
int main() {
    ...
    // maybe we could've done some clever top-to-bottom precomputation?
    // but this runs in < 1s
    for (auto& removed: bricks) {
        std::set<BrickPtr> falling;
        std::deque<BrickPtr> queue;
        queue.push_back(removed);
        while (!queue.empty()) {
            auto brick_ptr = queue.front();
            queue.pop_front();
            falling.insert(brick_ptr);
            for (auto& dep_ptr: brick_ptr->dependers) {
                std::vector<bool> support_is_falling;
                std::transform((dep_ptr->supports).begin(), (dep_ptr->supports).end(), 
                    std::back_inserter(support_is_falling), [&falling](BrickPtr b) {return falling.contains(b);});
                if (std::all_of(support_is_falling.cbegin(), support_is_falling.cend(), [](bool v){ return v;})) {
                    falling.insert(dep_ptr);
                    queue.push_back(dep_ptr);
                }
            }
        }
        ans += falling.size()-1;
